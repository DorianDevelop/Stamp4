<template>
	<Layout class="layout" routeAPI="/stamp3ate/user" :searchType="0" :formating="createJSONItem"
		...
		<template #default="props">
			<w-form class="editForm">
				<w-flex class="py2 align-end mb1 px1" gap="3">
					...
					<w-input label-color="green-dark1" class="xs3" label="Service" v-model="props.datas.service">
					</w-input>
					<v-select v-if="isAdmin" :options="levels" v-model="props.datas.level"
						:reduce="(level) => level.id"></v-select>
				</w-flex>

				<w-flex class="py2 align-end mb1 px1" gap="3">
					<w-input label-color="green-dark1" class="xs3" label="Code" v-model="props.datas.code"> </w-input>
					<w-input label-color="green-dark1" class="xs3" label="Pass" v-model="props.datas.pass"
						v-if="isAdmin"> </w-input>
				</w-flex>
