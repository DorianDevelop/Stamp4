...
exports.findNextIdPlug = (req, res) => {
	const db = createConnection('stamp3ate');
	const queryID = 'SELECT AUTO_INCREMENT FROM information_schema.tables WHERE table_name = "plug" AND table_schema = "stamp3ate";';
...
exports.createActionsOfPlug = (req, res) => {
	...
	const query = 'INSERT INTO `action`(`idPlug`, `target`, `funct`, `organ`, `action`, `order`, `label`, `idDriver`, `scpi`, `info`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
